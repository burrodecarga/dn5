    ...
    <div class="py-6">
        <div class="mx-auto sm:px-6 lg:px-8 bg-red-200" style="max-width:70%">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <h1 class="text-bold text-center text-gray-800 py-2 uppercase">Editar Item del subtema:
                    {{$item->subtema->titulo}}</h1>
                <hr>
                <div class="mt-0 md:mt-0">
                    <form action="{{route('items.update',$item->id)}}" method="POST" enctype="multipart/form-data"
                        ...
                        @csrf
                        @method('PUT')
                        <input type="hidden" name="subtema_id" value="{{$item->subtema->id}}">
